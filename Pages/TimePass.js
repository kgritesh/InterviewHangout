...
var TimePass = (function() {
  ...
  function TimePass(place, givenOptions) {
    // Determine effective options based on given values and defaults.
    var options = {}, defaults = TimePass.defaults;
    ...
    wrapper.className = "TimePass" + (options.lineWrapping ? " TimePass-wrap" : "");
    // This mess creates the base DOM structure for the editor.
    wrapper.innerHTML =
      '<div style="overflow: hidden; position: relative; width: 3px; height: 0px;">' + // Wraps and hides input textarea
        '<textarea style="position: absolute; padding: 0; width: 1px; height: 1em" wrap="off" ' +
          'autocorrect="off" autocapitalize="off"></textarea></div>' +
      '<div class="TimePass-scrollbar">' + // The vertical scrollbar. Horizontal scrolling is handled by the scroller itself.
        '<div class="TimePass-scrollbar-inner">' + // The empty scrollbar content, used solely for managing the scrollbar thumb.
      '</div></div>' + // This must be before the scroll area because it's float-right.
      '<div class="TimePass-scroll" tabindex="-1">' +
        '<div style="position: relative">' + // Set to the height of the text, causes scrolling
          '<div style="position: relative">' + // Moved around its parent to cover visible view
            '<div class="TimePass-gutter"><div class="TimePass-gutter-text"></div></div>' +
            // Provides positioning relative to (visible) text origin
            '<div class="TimePass-lines"><div style="position: relative; z-index: 0">' +
              // Used to measure text size
              '<div style="position: absolute; width: 100%; height: 0; overflow: hidden; visibility: hidden;"></div>' +
              '<pre class="TimePass-cursor">&#160;</pre>' + // Absolutely positioned blinky cursor
              '<pre class="TimePass-cursor" style="visibility: hidden">&#160;</pre>' + // Used to force a width
    ...
    try { stringWidth("x"); }
      ...
      if (e.message.match(/runtime/i))
        e = new Error("A TimePass inside a P-style element does not work in Internet Explorer. (innerHTML bug)");
      throw e;
    }
    ...
    // local functions in the TimePass function. Some do some extra
    // range checking and/or clipping. operation is used to wrap the
    // call so that changes it makes are tracked, and the display is
    // updated afterwards.
    var instance = wrapper.TimePass = {
      getValue: getValue,
      ...
      toggleOverwrite: function() {
        if(overwrite){
          overwrite = false;
          cursor.className = cursor.className.replace(" TimePass-overwrite", "");
        } else {
          overwrite = true;
          cursor.className += " TimePass-overwrite";
        }
      },
    ...
    function onFocus() {
      ...
      if (!focused) {
        ...
        if (scroller.className.search(/\bCodeMirror-focused\b/) == -1)
          scroller.className += " TimePass-focused";
    ...
    function onBlur() {
      if (focused) {
        ...
        scroller.className = scroller.className.replace(" TimePass-focused", "");
      }
    ...
    function overlapScrollbars() {
      ...
      tmpSb.className = "TimePass-scrollbar";
      tmpSb.style.cssText = "position: absolute; left: -9999px; height: 100px;";
      tmpSbInner.className = "TimePass-scrollbar-inner";
      ...
      if (collapsed) {
        ...
        function add(left, top, right, height) {
          var rstyle = quirksMode ? "width: " + (!right ? clientWidth : clientWidth - right - left) + "px"
                                  : "right: " + right + "px";
          html += '<div class="TimePass-selected" style="position: absolute; left: ' + left +
    ...
    function loadMode() {
      mode = TimePass.getMode(options, options.mode);
    ...
    function wrappingChanged(from, to) {
      if (options.lineWrapping) {
        wrapper.className += " TimePass-wrap";
        ...
        wrapper.className = wrapper.className.replace(" TimePass-wrap", "");
        maxLine = ""; maxLineChanged = true;
        doc.iter(0, doc.size, function(line) {
    ...
    var tempId = "TimePass-temp-" + Math.floor(Math.random() * 0xffffff).toString(16);
    ...
    function matchBrackets(autoclear) {
      ...
      var style = found.match ? "TimePass-matchingbracket" : "TimePass-nonmatchingbracket";
  ...
  var modes = TimePass.modes = {}, mimeModes = TimePass.mimeModes = {};
  TimePass.defineMode = function(name, mode) {
    if (!TimePass.defaults.mode && name != "null") TimePass.defaults.mode = name;
  ...
  TimePass.defineMIME = function(mime, spec) {
    mimeModes[mime] = spec;
  };
  TimePass.resolveMode = function(spec) {
    if (typeof spec == "string" && mimeModes.hasOwnProperty(spec))
      spec = mimeModes[spec];
    else if (typeof spec == "string" && /^[\w\-]+\/[\w\-]+\+xml$/.test(spec))
      return TimePass.resolveMode("application/xml");
    if (typeof spec == "string") return {name: spec};
    else return spec || {name: "null"};
  };
  TimePass.getMode = function(options, spec) {
    var spec = TimePass.resolveMode(spec);
    var mfactory = modes[spec.name];
    if (!mfactory) return TimePass.getMode(options, "text/plain");
    return mfactory(options, spec);
  };
  TimePass.listModes = function() {
    var list = [];
    for (var m in modes)
      if (modes.propertyIsEnumerable(m)) list.push(m);
    return list;
  };
  TimePass.listMIMEs = function() {
    var list = [];
  ...
  var extensions = TimePass.extensions = {};
  TimePass.defineExtension = function(name, func) {
    extensions[name] = func;
  };

  var commands = TimePass.commands = {
  ...
  TimePass.fromTextArea = function(textarea, options) {
    if (!options) options = {};
    ...
    var instance = TimePass(function(node) {
  ...
  TimePass.copyState = copyState;
  function startState(mode, a1, a2) {
    return mode.startState ? mode.startState(a1, a2) : true;
  }
  TimePass.startState = startState;
  ...
  TimePass.StringStream = StringStream;
  ...
  TimePass.e_stop = e_stop;
  TimePass.e_preventDefault = e_preventDefault;
  TimePass.e_stopPropagation = e_stopPropagation;
  ...
  TimePass.connect = connect;

  function Delayed() {this.id = null;}
  Delayed.prototype = {set: function(ms, f) {clearTimeout(this.id); this.id = setTimeout(f, ms);}};

  var Pass = TimePass.Pass = {toString: function(){return "TimePass.Pass";}};
  ...
  TimePass.htmlEscape = htmlEscape;
  ...
  TimePass.splitLines = splitLines;
  ...
  TimePass.defineMode("null", function() {
    return {token: function(stream) {stream.skipToEnd();}};
  });
  TimePass.defineMIME("text/plain", "null");
